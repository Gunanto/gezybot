...
- **Ordre des sections = ordre des piliers** : (1) agents persistants qui se souviennent → (2) plateforme self-hosted & self-improving → (3) UI d'agents → (4) onboarding Queenie → (5) omnicanal → (+) confiance/transparence. La nav suit ce fil narratif, pas l'ordre alphabétique ni l'ordre des tables SQL.
- **Catégorie** : la doc parle de « plateforme d'agents autonomes ». On évite le vocabulaire « front de chat » / « nouvelle conversation ».
- **Le terme « Agent »** est introduit une seule fois, tôt, dans *Concepts clés*, puis utilisé librement. En surface (titres de section, intro), on garde « agents » quand ça aide un nouveau venu.
- **Ton** : grand-public rassurant, concret, « ce que tu peux faire ». Honnêteté sur la maturité (renvoi vers une page *Limites & roadmap*).
- **Réutiliser le scaffold existant** : `getting-started/`, `agents/`, `channels/`, `mini-apps/`, `plugins/`, `providers/`, `memory/`, `api/`, `guides/` existent déjà. Le plan les réorganise sous les piliers sans casser les slugs déjà écrits.
...
Pilier 4 — Onboarding & configuration (Queenie)
  Queenie, votre configurateur
...
| `getting-started/first-launch-queenie` | Le parcours 3 écrans puis passage en chat avec Queenie. | `queenie.md` · `catalogue §5` |
| `getting-started/configuration` *(existe)* | Brancher un provider LLM, réutilisation de clé = N capacités. | `config.md` · `queenie.md` · `providers/supported.md` |
| `getting-started/first-agent` *(existe)* | Créer/nommer/spécialiser un premier Agent, choisir un modèle. | `idea.md` · `prompt-system.md` |

### Pilier 1 — Vos agents persistants

| Page | Scope | Source |
|---|---|---|
| `agents/overview` *(existe)* | Identité, expertise, caractère, modèle ; un Agent partagé par tous les utilisateurs. | `catalogue §1` · `prompt-system.md` · `idea.md` |
| `agents/continuous-session` | « Jamais de nouvelle conversation » : une session continue par Agent, queue FIFO + priorité utilisateur. | `idea.md` · `catalogue §1` |
| `agents/memory` *(existe)* / `memory/how-it-works` *(existe)* | Comment les Agents se souviennent : dual-channel, recherche hybride, catégories. | `catalogue §14` · `memory/*` · `agent-context-journal.md` · `compacting.md` |
| `agents/sub-agents` | Déléguer du travail à des sous-Agents éphémères (await vs async, profondeur). | `catalogue §1, §15` · `idea.md` |
| `agents/collaboration` | Communication inter-Agent request/reply, corrélation, anti-boucle. | `catalogue §1` · `teams-design.md` · `idea.md` |
| `agents/contacts` | Registre de contacts unifié (Agents + humains), notes scopées. | `contacts.md` · `catalogue §16` |
...
| Page | Scope | Source |
|---|---|---|
| `agents/tools` *(existe)* | Les outils natifs : ce qu'un Agent peut faire out-of-the-box. | `idea.md` · `catalogue §1, §16` |
...
| Page | Scope | Source |
|---|---|---|
| `ui/tour-pwa` | Tour de l'UI (sidebar Agents, fil, viewers) + installation PWA, offline. | `frontend-perf-journal.md` · `catalogue §2` · `DesignSystemPage.tsx` |
...
| Page | Scope | Source |
|---|---|---|
| `queenie/overview` | Queenie = configurateur conversationnel permanent ; ce qu'il peut/ne peut pas faire. | `queenie.md` · `queenie-knowledge.md` · `catalogue §5` |
| `queenie/global-rules-defaults` | Règles globales injectées à chaque Agent + defaults par capacité. | `queenie.md` · `prompt-system.md` · `catalogue §5` |
...
| Page | Scope | Source |
|---|---|---|
| `channels/overview` *(existe)* | Modèle omnicanal : 6 plateformes, liaison Agent↔canal mutable, identité. | `catalogue §12` · `channel-files-journal.md` |
...
| `channels/transfer` | Transfert dynamique de canal entre Agents en temps réel + chaîne de causalité. | `catalogue §12` · `channel-files-journal.md` |
...
| `connected-accounts/oauth-imap-caldav` | Mécanique OAuth2 / IMAP / CalDAV, tokens jamais vus par les Agents. | `email.md` · `calendar.md` · `catalogue §8` |

### Automatisation

| Page | Scope | Source |
|---|---|---|
| `automation/crons` | Planifier des sous-Agents (POSIX/ISO8601, one-shot, learnings, approbation). | `catalogue §15` · `cron-manager-journal.md` · `guides/autonomy-quickstart.md` |
...
| Page | Scope | Source |
|---|---|---|
| `projects/overview` | Projets, contexte injecté, connaissances épinglées, projet actif par Agent. | `projects.md` · `catalogue §7` |
...
| Page | Scope | Source |
|---|---|---|
| `security/vault` | Coffre AES-256-GCM, références `$vault:`, saisie sécurisée, redaction. | `catalogue §13` · `SECURITY.md` · `queenie.md` |
| `security/multi-user` | Isolation multi-utilisateur, quick-sessions, notes scopées. | `catalogue §16` · `idea.md` |
| `transparency/tokens-costs` | Context Viewer, breakdown par section, « zéro surprise de coûts ». | `catalogue §11` · `agent-context-journal.md` |
| `transparency/cache-calibration` | Observabilité cache Anthropic + calibration EMA per-Agent. | `catalogue §11` · `compacting.md` |
...
|---|---|---|
| 1 | L'équipe de Agents en action (collaboration inter-agents) | `agents/collaboration` (+ `agents/sub-agents`) |
| 2 | Queenie : votre setup en conversation | `getting-started/first-launch-queenie` → `queenie/overview` |
...
| 6 | Des mini-apps construites par vos Agents | `mini-apps/overview` |
...
- **Agent** — un agent persistant avec une identité, une expertise, une mémoire et des outils. Partagé par tous les utilisateurs de l'instance. → `agents/overview`
- **Session continue** — un seul fil ininterrompu par Agent ; il n'y a pas de « nouvelle conversation ». Le contexte ancien est résumé, jamais supprimé. → `agents/continuous-session`
- **Mémoire** — savoir long terme hybride (sémantique + texte) que le Agent accumule automatiquement et via l'outil `memorize` ; catégories fact/preference/decision/knowledge. → `memory/how-it-works`
- **Sous-Agent (tâche)** — instance éphémère qu'un Agent spawn pour déléguer un travail ; mode `await` (le parent attend) ou `async` (fond). → `agents/sub-agents`
- **Toolbox** — liste nommée d'outils autorisés assignée à un Agent/une tâche, qui compose ses capacités par-dessus un socle obligatoire. → `extending/toolboxes`
- **Channel binding** — la liaison mutable entre un Agent et un canal de messagerie ; on peut transférer le canal d'un Agent à l'autre sans changer d'adresse. → `channels/transfer`
...
*(Entrées secondaires possibles si la place le permet : Compacting, Scout, Mini-app, Provider, Queenie — mais les 7 ci-dessus sont le minimum demandé et suffisent au glossaire 1.0.)*
...
4. `getting-started/first-launch-queenie`
5. `getting-started/configuration`
6. `getting-started/first-agent`

**Vague 2 — Les 8 héros (parité marketing, le « pourquoi »)**
7. `memory/how-it-works` (héros 3)
8. `agents/collaboration` + `agents/sub-agents` (héros 1)
9. `queenie/overview` (héros 2)
...
15. `agents/overview`, `agents/continuous-session`, `agents/context-compacting`, `agents/contacts`
...
22. `queenie/global-rules-defaults`, `admin/platform`
...
- **Réemploi du scaffold** : les pages marquées *(existe)* sont déjà dans `docs-site/src/content/docs/`. Vague 1-2 = surtout réorganiser la sidebar (`astro.config`/sidebar Starlight) selon §1 + écrire les nouvelles pages `intro/*`, `agents/*`, `queenie/*`, `security/*`, `transparency/*`, `connected-accounts/*`, `automation/*`, `projects/*`, `ui/*`.
...
- **Ne pas publier en l'état** : `idea.md`, `schema.md`, `api.md`, `sse.md`, `prompt-system.md`, `compacting.md`, `queenie.md`, les `*-journal.md`, `teams-design.md` — ce sont des sources internes à reformuler en langage bénéfice-d'abord.
- **Pages clés à signaler comme prioritaires hors-vague si le temps manque** : le glossaire (`intro/concepts`) et `getting-started/first-launch-queenie` portent à eux seuls la compréhension + l'activation du produit.
