...
// ─── applyPreToolTextStripping ───────────────────────────────────────────────
//
// Re-implementation of applyPreToolTextStripping following the exact source
// logic (same pattern as maskOldToolResults: importing kin-engine.ts triggers
// heavy module side-effects, so we re-test the contract here).

function applyPreToolTextStripping(args: {
  fullContent: string
  stepText: string
...
describe('applyPreToolTextStripping', () => {
  it('strips step text and collapses tool offsets when step has both text and a tool call', () => {
    const stepText = 'Voilà le résultat de la commande:\n```\nfake output\n```\n'
    const fullContent = 'Prior turn text. ' + stepText
    const stepStartLength = 'Prior turn text. '.length
    const stepToolCalls = [{ offset: fullContent.length }]

    const result = applyPreToolTextStripping({
      fullContent,
      stepText,
      stepStartLength,
      stepToolCalls,
    ...
    expect(result.strippedLength).toBe(stepText.length)
    expect(result.fullContent).toBe('Prior turn text. ')
    expect(stepToolCalls[0]!.offset).toBe(stepStartLength)
  })

  it('is a no-op when the step has a tool call but emitted no text', () => {
    const fullContent = 'Already accumulated content.'
    const stepToolCalls = [{ offset: fullContent.length }]

    const result = applyPreToolTextStripping({
      fullContent,
      stepText: '',
      stepStartLength: fullContent.length,
      stepToolCalls,
    })

    expect(result.stripped).toBe(false)
    expect(result.strippedLength).toBe(0)
    expect(result.fullContent).toBe(fullContent)
    expect(stepToolCalls[0]!.offset).toBe(fullContent.length)
  })

  it('is a no-op when the step has text but no tool call (the legitimate final answer)', () => {
    const fullContent = 'Here is what the tool returned: 42.'
    const stepText = 'Here is what the tool returned: 42.'
  ...
  it('preserves text from prior tool-less steps when stripping the current step', () => {
    const priorStepText = 'OK, je vérifie maintenant le contenu du fichier.\n'
    const currentStepText = 'Le fichier contient: lorem ipsum.\n'
    const fullContent = priorStepText + currentStepText
    const stepStartLength = priorStepText.length
    const stepToolCalls = [
    ...
    expect(result.stripped).toBe(true)
    expect(result.fullContent).toBe(priorStepText)
    expect(stepToolCalls[0]!.offset).toBe(stepStartLength)
    expect(stepToolCalls[1]!.offset).toBe(stepStartLength)
  })
})
