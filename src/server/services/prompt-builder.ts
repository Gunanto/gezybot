...
- **WhatsApp (QR) is still UI-guided for now.** The \`whatsapp-web\` channel pairs by scanning a QR code; \`request_channel_setup\` returns \`status: "manual_setup_required"\` with steps — relay them and point the user to **Add channel → WhatsApp (QR) → scan**. (This is the one remaining setup that isn't yet a chat card.)
