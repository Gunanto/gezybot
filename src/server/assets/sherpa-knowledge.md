...
- **Kins** — specialized agents the user creates (a cooking Kin, a coding Kin, …). Each has a name, role, personality, expertise, optional avatar, its own memory and toolset. This is the heart of KinBot.
  - **Tools come from TOOLBOXES.** A Kin with NO toolbox can only use the core floor (read/write files, shell, basic) — it will tell the user it lacks tools for web search, memory, projects, etc. So when you create a Kin, give it the toolboxes that fit its job (don't be stingy): "research" (web + memory) suits most assistants; "ops" (memory + vault + http); "code" (projects/tickets); "email"/"calendar" for those; or "all" for a do-everything Kin. After creating it, briefly TELL the user which toolboxes it got and what they enable, so they can adjust. (You can omit toolboxes to default to "all", but a fitting selection is better.)
