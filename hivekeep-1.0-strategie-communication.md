...
`[À TRANCHER]` Le mot "Agent" est central dans le produit mais opaque pour un nouveau visiteur. Faut-il l'introduire dès la tagline, ou d'abord parler d'"agents" puis révéler "Agents" ? Je penche pour : **"agents" en surface marketing, "Agents" comme terme propriétaire révélé juste après** (effet de marque sans friction de compréhension).
...
- **Message qui résonne :** *"Une session continue partagée : tout le monde parle aux mêmes Agents, qui connaissent votre contexte, répondent dans la langue de chacun, et se passent le relais entre eux."*
...
UI tout-en-un polie/PWA · extensibilité (custom tools + mini-apps + plugins) · onboarding Queenie · avatars.
...
| **4** | **Setup conversationnel, zéro YAML (Queenie)** | "Pas de fichier de config. Un agent vous guide : il branche vos providers, gère vos secrets en sécurité, crée vos premiers agents — par conversation." | Onboarding Queenie (3 écrans → chat) · secure input (secrets jamais au LLM) · reste accessible à vie · avatars auto-générés personnalisables | Non-techniques, grand public prudent |
| **5** | **Vos agents, partout — pas seulement dans une app** | "Telegram, WhatsApp, Slack, Discord, Signal, Matrix — plus la PWA. Un Agent peut passer le canal à un spécialiste en temps réel." | 6 plateformes natives · transfert de canal dynamique entre Agents · livraison causal-chain | Power-users, petites équipes, mobiles |
...
- **Queenie : remonté à #4 (et non #5) sur demande du fondateur.** « Onboarding facile » fait partie de ses arguments prioritaires. C'est un *gap d'adoption majeur* (aucun self-hosted n'a ça, `wow_factor` 5) et il sert directement le ton grand-public-rassurant validé. Reste aussi une **feature héros #1 en démo** (cf. section 5) : *priorité de pitch ≠ priorité de showcase.*
- **Extensibilité : fusionnée dans le pilier #2 "self-hosted & self-improving".** Plutôt qu'un pilier "extensibilité" isolé, on raconte que la plateforme **s'améliore elle-même** — les Agents créent outils, mini-apps et plugins. Custom tools à rendu riche + mini-apps restent inédits dans le self-hosted (gap veille). Bon instinct du fondateur, recadré dans un récit plus fort.
- **UI/PWA : maintenue comme pilier (#3), pas reléguée.** Recadrage de catégorie (§0) : différenciante dans l'arène des agents autonomes. Elle infuse aussi tous les autres piliers (c'est *grâce à* elle que Queenie, les mini-apps, les avatars brillent).
...
`[À TRANCHER]` Mon pilier #1 ("équipe persistante qui se souvient") parie que la **mémoire + collaboration** est le vrai différenciateur durable, en phase avec la tendance de fond de la veille ("la demande se déplace de la génération de code vers la mémoire persistante et l'automatisation ambiante"). Le fondateur doit valider ce pari narratif : vend-on d'abord *le polish* (UI/Queenie/avatars) ou *la substance* (mémoire/agents/souveraineté) ? Je recommande fortement la substance, avec le polish comme preuve.
...
| Onboarding conversationnel (zéro YAML) | ✅ Queenie | ❌ | ❌ | ❌ | ❌ | ❌ | ⚠️ CLI onboard | n/a |
...
1. **L'équipe de Agents en action (collaboration inter-agents)**
   *Démo :* GIF — un Agent "Recherche" envoie un `request` à un Agent "Rédaction", qui produit le texte, sans intervention humaine entre les deux. Badges de queue visibles, avatars distincts.
   *Pourquoi héros :* incarne le pilier #1, impossible à montrer chez les concurrents.

2. **Queenie : votre setup en conversation**
   *Démo :* GIF/vidéo — fresh install → 3 écrans rapides → Queenie salue, demande une clé via popup sécurisée (le secret n'apparaît JAMAIS dans le chat), teste le provider, propose un premier Agent. ~30s.
...
4. **Vos agents, partout (omnicanal + transfert de canal)**
   *Démo :* un user écrit sur Telegram "parle-moi crypto" → le Agent généraliste appelle `transfer_channel` → le badge change en direct, le spécialiste reprend avec le contexte du handoff. Montrer la grille des 6 logos plateformes.
...
6. **Des mini-apps construites par vos Agents**
   *Démo :* `create_mini_app` template dashboard → 50+ composants rendus, dark mode toggle côté parent → re-render instantané. Puis "Improve this" : suggestion en langage naturel → le Agent édite → reload.
...
8. **Zéro surprise de coûts (transparence tokens/contexte)**
   *Démo :* le Context Viewer — barre stacked multicolore (système/mémoires/messages/outils), breakdown par section, panel cache Anthropic (hit rate, TTL), calibration EMA per-Agent.
   *Pourquoi héros :* personne grand public n'expose ça ; rassure les budget-conscious.

**Sections "preuve" plus légères (pas pleine largeur, mais présentes) :** avatars auto-générés (galerie de 6 Agents), 18 palettes (palette switcher live), projets/Kanban + GitHub worktrees, crons/webhooks/human-in-the-loop, comptes connectés (mail/cal), PWA installable.
...
/features               Toutes les capacités, regroupées par pilier (agents · self-host · extensibilité · omnicanal · onboarding)
  /features/agents          Agents, mémoire, sous-Agents, collaboration, session continue
...
/quickstart             docker run, prérequis, première connexion, lien Queenie
...
3. **Hook visuel** : un GIF unique en haut (le plus fort = Queenie onboarding OU collaboration inter-Agent). Un seul, qui claque.
...
4. **Pitch 30 secondes** (3-4 puces, pas un paragraphe) :
   ...
   - 🤝 **Une équipe, pas un chatbot** — vos Agents collaborent et spawnent des sous-agents.
...
5. **Quickstart** immédiatement après — le `docker run` complet, copiable, puis "ouvrez votre navigateur, Queenie s'occupe du reste." Insister sur l'absence d'étape de config manuelle.

6. **Capture d'écran** de l'UI (chat + sidebar Agents) sous le quickstart.

7. Puis : Features (liens vers site/docs) · Comparatif condensé · Architecture en une image · Contribuer · Licence.

**Ton du README :** confiant, concret, orienté "ce que tu peux faire en 2 minutes". Éviter le jargon interne (Agent/sous-Agent) dans les 3 premières lignes — l'introduire juste après le hook.
...
```
Getting Started     Install (Docker/manuel) · Premier lancement · Queenie · Concepts clés (Agent, session, mémoire)
Core Concepts       Agents & identité · Session continue & compacting · Mémoire · Queue & priorité · Contacts
Working with Agents   Créer/configurer · Toolboxes · Avatars · Modèles & providers
Collaboration       Sous-Agents (await/async) · Inter-Agent messaging · Crons · Webhooks · Human-in-the-loop
...
**Branding & nom**
- Le mot **"Agent"** : on l'expose dès la tagline ou on le révèle après "agents" ? (Je recommande : agents en surface, Agents juste après.)
- **Queenie** : nom définitif du configurateur ? Risque de collision marque (matériel de montagne, autres SaaS "Queenie"). À vérifier juridiquement.
- Identité visuelle de la marque Hivekeep elle-même (logo, mascotte ?) — l'avatar bundlé de Queenie peut-il devenir un élément de marque ?
...
- Validation du **réordonnancement des piliers** : la substance (mémoire/agents/souveraineté) en #1-2 plutôt que le polish (UI/Queenie/avatars). C'est le pari le plus important du document.
