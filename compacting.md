...
Compacting is evaluated **after each LLM turn** (after the Kin has responded). It uses a **token-percentage-based** trigger:
...
The summary is generated by a dedicated LLM call (model configurable via `COMPACTING_MODEL`, per-Kin override, or the Kin's own model).
...
When a Kin has accumulated a large backlog (e.g. context at 95% after a long conversation), a single compaction might not be enough. `maybeCompact()` runs in a **loop**: it keeps triggering compaction until the context usage drops below the threshold, up to a maximum of 5 cycles. SSE progress events include the current cycle and estimated total so the UI can show progress.

---

## Force compact

Users can trigger compaction manually from the UI (Kin settings → Compaction tab). Force compact bypasses the threshold check and runs `runCompacting()` directly. If there aren't enough messages to compact (< 2 messages outside the keep window), a `NOTHING_TO_COMPACT` error is persisted in the conversation history.
